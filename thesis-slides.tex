...
\begin{frame}{PNT Benchmark: Extracted Graph}
\scriptsize
\centering
\resizebox{0.86\textwidth}{!}{%
\begin{tabular}{lr}
...
\vspace{0.8em}
\small
The benchmark is much larger than the target set: $92$ sorry'd targets sit inside a $6{,}563$-declaration extracted dependency graph.
\end{frame}

\begin{frame}{PNT Benchmark: Reuse and Harness}
\scriptsize
\centering
\resizebox{0.86\textwidth}{!}{%
\begin{tabular}{lr}
...
\vspace{0.8em}
\small
Systems fill sorry'd targets under a fixed budget; Agora variants add verified publication, market bounties, and optional learned strategy recommendations.
\end{frame}
...
\node[target] (top) {\texttt{SmallSquare}\\\texttt{InRectangle}\\$\alpha_2,\approx2850\,\mathrm{s}$};
\node[target,below left=0.9cm and 1.0cm of top] (nhds) {\texttt{nhds\_has}\\\texttt{Basis\_square}\\$\alpha_3,\approx2550\,\mathrm{s}$};
\node[target,below right=0.9cm and 1.0cm of top] (sqsub) {\texttt{square\_subset}\\\texttt{\_square}\\$\alpha_1,\approx2400\,\mathrm{s}$};
...
\node[conj,below=0.9cm of sqsub] (rectsub) {\texttt{RectSubRect'}\\$\alpha_1,\approx2300\,\mathrm{s}$};
\node[target,below=0.9cm of rectsub] (iff) {\texttt{rect\_subset\_iff}\\$\alpha_2,\approx2100\,\mathrm{s}$};
\node[conj,below left=0.8cm and 0.25cm of iff] (left) {\texttt{left\_mem}\\\texttt{\_rect}\\$\alpha_2,\approx1500\,\mathrm{s}$};
\node[conj,below right=0.8cm and 0.25cm of iff] (right) {\texttt{right\_mem}\\\texttt{\_rect}\\$\alpha_2,\approx1500\,\mathrm{s}$};
\draw[arr] (top) -- (nhds);
\draw[arr] (top) -- (sqsub);
...
\begin{frame}[fragile]{Listing 1: Helper Lemmas, $\alpha_2$, $\approx1500\,\mathrm{s}$}
\scriptsize
...
\begin{frame}[fragile]{Listing 2: First Target, $\alpha_2$, $\approx2100\,\mathrm{s}$}
\scriptsize
...
\begin{frame}[fragile]{Listing 3: Utility Lemma, $\alpha_1$, $\approx2300\,\mathrm{s}$}
\scriptsize
...
\begin{frame}[fragile]{Listing 4: Square Monotonicity, $\alpha_1$, $\approx2400\,\mathrm{s}$}
\scriptsize
...
\begin{frame}[fragile]{Listing 5: Hard Branch, $\alpha_3$, $\approx2550\,\mathrm{s}$}
\scriptsize
...
\begin{frame}[fragile]{Listing 6: Final Target, $\alpha_2$, $\approx2850\,\mathrm{s}$}
\scriptsize
...
\begin{itemize}
  ...
  \item $\alpha_2$ conjectures and proves \texttt{left\_mem\_rect} and \texttt{right\_mem\_rect} at about $1500\,\mathrm{s}$.
...
\midrule
$\approx1500$ & $\alpha_2$ & \texttt{left\_mem\_rect} & conjectured & --- \\
$\approx1500$ & $\alpha_2$ & \texttt{right\_mem\_rect} & conjectured & --- \\
$\approx2100$ & $\alpha_2$ & \texttt{rect\_subset\_iff} & target & helpers \\
$\approx2300$ & $\alpha_1$ & \texttt{RectSubRect'} & conjectured & \texttt{rect\_subset\_iff} \\
$\approx2400$ & $\alpha_1$ & \texttt{square\_subset\_square} & target & \texttt{RectSubRect'} \\
$\approx2550$ & $\alpha_3$ & \texttt{Complex.nhds\_hasBasis\_square} & target & Mathlib \\
$\approx2850$ & $\alpha_2$ & \texttt{SmallSquareInRectangle} & final target & both branches \\
\bottomrule
\end{tabular}}

\vspace{0.8em}
\small
The trace shows parallelism from $1500$--$2550$ s, conjecture as task decomposition, and publication-triggered cascades.
\end{frame}
