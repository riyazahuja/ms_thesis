...
%       Packages & Setup
% =====================================
\usepackage[insane]{cmu-thesis} % Proper Thesis Formatting. 'insane' disables the template's strict draft-structure checks
...
\usepackage{amsthm}            % For matrices
...
\usepackage{circuitikz}         % For TikZ/circuit diagrams
\usetikzlibrary{arrows.meta}
\usetikzlibrary{shapes, positioning, calc, fit, backgrounds, quotes}
\usepackage{pgf}                % For pgfonlayer (used in pipeline diagram)
...
\usepackage{paracol}            % For side-by-side appendix listings
\usepackage{fvextra} % Needed for verbatim in multicol
\usepackage{algorithm}          % For algorithm environments
\usepackage{algorithmic}        % For algorithmic pseudocode
\usepackage{bm}                 % For bold math in code listings
\usepackage{stmaryrd}           % For \llbracket and \rrbracket in code listings
...
% \subtitle{Multiagent Reinforcement Learning and Applications to Formal Mathematics}

% >>>>>>>>> PERSONAL INFORMATION <<<<<<<<<
\degreetitle{Master of Science}     % Degree Title like Master of Science or Doctor of Philosophy
\discipline{Mathematics}    % Area of research, eg. Applied Physics, Computer Science
...
\author{Riyaz Ahuja}        % your NAME, don't forget your middle names.

% Optional Metadata for Title Page
\thesisdate{April 16, 2026}
% Thesis committee (declare individually), you can also include their affliations, if they are not faculty members at CMU
\committeememberone{Dr. Prasad Tetali}
\committeemembertwo{Dr. Jeremy Avigad}
\committeememberthree{Dr. Sean Welleck}
...
\input{chapter/i-acknowledgments}
\end{acknowledgments} \newpage
...
\input{chapter/02-RelatedWork}
\input{chapter/03-ProofOptimization}
\input{chapter/04-ImProver}
\input{chapter/05-ImProverEval}
\input{chapter/06-ImProver2}
\input{chapter/07-ImProver2Eval}
\input{chapter/08-Conclusion}
...
\input{chapter/A1-ConfigImProver.tex}
\input{chapter/A2-QualImProver.tex}
\input{chapter/B1-ConfigImProver2.tex}
\input{chapter/B2-QualImProver2.tex}
