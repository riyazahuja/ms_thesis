...
\textbf{Original (human-written)}
\begin{scriptsize}
\begin{lstlisting}[basicstyle=\scriptsize\ttfamily,literate={lbrak}{$\llbracket$}1 {rbrak}{$\rrbracket$}1 ]
...
/-- If `f(p(t) = g(q(t))` for two paths `p` and `q`, then the induced path homotopy classes
`f(p)` and `g(p)` are the same as well, despite having a priori different types -/
theorem heq_path_of_eq_image : HEq ((πₘ f).map $\llbracket$p$\rrbracket$) ((πₘ g).map $\llbracket$q$\rrbracket$)    := by
