...
We again employ the Chain-of-States technique described in \ref{ssec:cos_imp} to expose the internal proof states of the original proof $y_0$ to the model. As shown in ImProver, his provides a richer signal about the structure of the proof and the effect of each tactic than is available from the original proof text alone, which may be more difficult for the model to parse and understand.
...
The details of the prompting and generation process for each of these channels are described in Appendix~\ref{app:informal}.
